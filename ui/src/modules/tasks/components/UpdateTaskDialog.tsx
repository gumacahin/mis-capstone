...
export default function UpdateTaskDialog({
  ...
  useEffect(() => {
    ...
    // Store the original URL in a variable to prevent stale closure
    const originalUrlValue = originalUrl.current;

    // Restore original URL when dialog closes
    return () => {
      window.history.replaceState(null, "", originalUrlValue);
    };
  }, [task.id]);
